...
function Asteroid(x, y, mass, angle, x_speed, y_speed, rotation_speed) {
  var density = 1; // kg per square pixel
  var radius = Math.sqrt(mass / density / Math.PI);
  this.super(x, y, mass, 0, angle, x_speed, y_speed, rotation_speed);
...
Asteroid.prototype.update = function(elapsed, context) {
...
function Projectile(mass, lifetime, x, y, x_speed, y_speed, rotation_speed) {
  var density = 0.1; // low density means we can see very light projectiles
  var radius = Math.sqrt(mass / density / Math.PI);
